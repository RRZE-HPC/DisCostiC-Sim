...
namespace DisCosTiC
{
	...
	public:
		...
		DisCosTiC::idNodeTypePair Oneway_PositiveDisplacement(DisCosTiC_Datatype process_Rank, UserInterface::YAMLParser YAML_args, DisCosTiC_Datatype N_size_Of_Cluster, MPI_Comm comm)
		{
            ...
            double message_size = CFG_args.getValue<long int>("message_size");
			...
			for (auto rank : DisCosTiC::getRange(systemsize))
			{
				...
				send = DisCosTiC->Isend(&rank, message_size, MPI_DOUBLE, top, 1, MPI_COMM_WORLD, &requests[0], comp);
				recv = DisCosTiC->Irecv(&rank, message_size, MPI_DOUBLE, top, 2, MPI_COMM_WORLD, &requests[1], comp);
				// }
				// if (rank > 0)
				// {
				int bottom = (rank == 0 ? systemsize - 1 : rank - 1);
				send = DisCosTiC->Isend(&rank, message_size, MPI_DOUBLE, bottom, 2, MPI_COMM_WORLD, &requests[2], comp);
				recv = DisCosTiC->Irecv(&rank, message_size, MPI_DOUBLE, bottom, 1, MPI_COMM_WORLD, &requests[3], comp);
				// }
