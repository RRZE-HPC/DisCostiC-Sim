	...
	public:
		...
		DisCosTiC::idNodeTypePair Oneway_PositiveDisplacement(DisCosTiC_Datatype process_Rank, UserInterface::YAMLParser YAML_args, DisCosTiC_Datatype N_size_Of_Cluster, MPI_Comm comm)
		{
			...
			for (auto rank : DisCosTiC::getRange(systemsize))
			{
				...
				comp = DisCosTiC->Exec("SRC:double r1; \n double res;\n double dx;\n double dy;\n double dx2;\n double dy2;\n double idx2;\n double idy2;\n double omega;\n double factor;\n double src[imax][imax];\n double rhs[imax][imax];\n for( int j=1; j<jmaxLocal+1; j++ ) {\n for( int i=1; i<imax+1; i++ ) {\n r1 = rhs[j][i] - ((src[j][i-1] - 2.0 * src[j][i] + src[j][i+1]) * idx2 + (src[j-1][i] - 2.0 * src[j][i] + src[j+1][i]) * idy2);\n src[j][i] = src[j][i] - (factor * r1);\n res = res + (r1 * r1);\n }\n }\n//BREAK:" + CFG_args.getValue<std::string>("benchmark_kernel") + "//./nodelevel/machine-files/" + arch_name + ".yml//" + std::to_string(ECM_core) + "+" + std::to_string(remainder_cores) + "//-D imax " + std::to_string(imax) + " -D jmaxLocal " + std::to_string(jmaxLocal), recv, YAML_args, process_Rank, N_size_Of_Cluster, comm);
