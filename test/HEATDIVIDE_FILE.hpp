...
namespace DisCosTiC
{
	...
	public:
        ...
        comp = DisCosTiC->Exec("FILE:DIVIDE.c//BREAK:" + CFG_args.getValue<std::string>("benchmark_kernel") + "//./nodelevel/machine-files/" + arch_name + ".yml//" + std::to_string(ECM_core) + "+" + std::to_string(remainder_cores) + "//-D N " + std::to_string(dim_x) + " -D  s " + std::to_string(55), compADD, YAML_args, process_Rank, N_size_Of_Cluster, comm);
