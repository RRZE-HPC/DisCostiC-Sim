...
from kerncraft import models
from kerncraft import __version__
from kerncraft.kernel import KernelCode, KernelDescription, symbol_pos_int
...
def create_parser():
    ...
    parser.add_argument('--output', '-o', type=str, default=sys.stdout, 
                        help='Addes results to PICKLE file for later processing.')
...
def run(parser, args, extras, system_number, output_file=sys.stdout):
    ...
    for define in define_product:
        ...
        for model_name in uniquify(args.pmodel):
            ...
            # if(not isExist):
            report(model, extras, system_number, output_file=args.output)
...
def report(model, extras, system_number, output_file):
    with open(output_file, 'a') as f:
        if(system_number == "0"):
            print("", file=f)
            print("#ECM - Primary ", file=f)
            multiCore = str(model.results['multi-core']['performance']['cy/CL'])
            print('T_OL = {:.1f}'.format(model.results['ECM'][0]), file=f)
            print('T_nOL = {:.1f}'.format(model.results['ECM'][1][0]), file=f)
            print('T_L1L2 = {:.1f}'.format(model.results['ECM'][1][1]), file=f)
            print('T_L2L3 = {:.1f}'.format(model.results['ECM'][1][2]), file=f)
            print('T_L3Mem = {:.1f}'.format(model.results['ECM'][1][3]), file=f)
            print('T_ECM = {:.1f}'.format(max(model.results['ECM'][0], sum(model.results['ECM'][1]))), file=f)
            print('Scaling_cores = '+str(model.results['scaling cores']), file=f)
            # print('T_MECM = ' + multiCore.split(' ')[0], file=f)
            # print(extras)

            for i in range(1,len(model.results['scaling prediction'])+1):
                # print(i)
                for cores in extras:
                    if(i == int(cores)):
                        print('T_{}ECM = {:.1f}'.format(int(i),float(str(model.results['scaling prediction'][i-1]['performance']['cy/CL']).split(' ')[0])), file=f)
            print("", file=f)
        else:
            print("", file=f)
            print("#ECM - Secondary ", file=f)
            multiCore = str(model.results['multi-core']['performance']['cy/CL'])
            print('T_OL_Secondary = {:.1f}'.format(model.results['ECM'][0]), file=f)
            print('T_nOL_Secondary = {:.1f}'.format(model.results['ECM'][1][0]), file=f)
            print('T_L1L2_Secondary = {:.1f}'.format(model.results['ECM'][1][1]), file=f)
            print('T_L2L3_Secondary = {:.1f}'.format(model.results['ECM'][1][2]), file=f)
            print('T_L3Mem_Secondary = {:.1f}'.format(model.results['ECM'][1][3]), file=f)
            print('T_ECM_Secondary = {:.1f}'.format(max(model.results['ECM'][0], sum(model.results['ECM'][1]))), file=f)
            print('Scaling_cores_Secondary = '+str(model.results['scaling cores']), file=f)
            # print('T_MECM = ' + multiCore.split(' ')[0], file=f)
            # print(extras)
            ...
            for i in range(1,len(model.results['scaling prediction'])+1):
                ...
                for cores in extras:
                    if(i == int(cores)):
                        print('T_{}ECM_Secondary = {:.1f}'.format(int(i),float(str(model.results['scaling prediction'][i-1]['performance']['cy/CL']).split(' ')[0])), file=f)
            print("", file=f)
...
def main():
    ...
    for el in sys.argv:
        if "-extras" in el:
            ...
            system_number = copy.deepcopy(el)
            remove_list.append(el)



    sys.argv = list(filterfalse(remove_list.__contains__, sys.argv))

    
    extras=extras.replace('-extras','')
    extras=extras.split('+')
    if '0' in extras: 
        extras.remove('0')

    system_number=system_number.replace('-systemNo','')
    ...
    run(parser, args, extras, system_number)
